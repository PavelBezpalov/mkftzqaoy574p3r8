...
local function gcd()
  ...
  if ability_to_cast and ability_to_cast > 0 and (ability_to_cast == SB.FireBlast or ability_to_cast == SB.Combustion) then
    return cast_while_casting(ability_to_cast, player)
  end
...
local function combat()
  local healing_potion = bxhnz7tp5bge7wvu.settings.fetch('fire_nikopol_healing_potion', false)
  local trinket_13 = bxhnz7tp5bge7wvu.settings.fetch('fire_nikopol_trinket_13', false)
  local trinket_14 = bxhnz7tp5bge7wvu.settings.fetch('fire_nikopol_trinket_14', false)
  local ability_to_cast = Hekili_GetRecommendedAbility( "Primary", 1 )
  ...
  if modifier.lalt and hekili_castable(SB.BlazingBarrier) then
    return cast_with_queue(SB.BlazingBarrier)
  end
  
  if toggle('auto_shield', false) and player.buff(SB.BlazingBarrier).down and hekili_castable(SB.BlazingBarrier) then
    return cast_with_queue(SB.BlazingBarrier)
  end
  ...
  if target.enemy and target.alive then  
    ...
    if ability_to_cast and ability_to_cast > 0 then
      if ability_to_cast == SB.Meteor or ability_to_cast == SB.Flamestrike then
...
local function interface()
    ...
    show = false,
    template = {
      { type = 'header', text = 'Fire Settings' },
      { type = 'rule' },   
  ...
  configWindow = bxhnz7tp5bge7wvu.interface.builder.buildGUI(fire_gui)
